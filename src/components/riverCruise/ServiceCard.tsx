...
const ServiceCard = ({ item, height = "h-[25vw]", isOpen, onClick, colSpan }: { item: ICruise; height: string; isOpen: boolean; onClick: () => void; colSpan: string }) => {
			...
			{isOpen && (
				<div className="h-full flex flex-col justify-evenly gap-4 py-4 lg:px-6">
					<div className="space-y-4">
						<p className="text-sm font-semibold">Price Start $ {item.price}</p>
						<p className="text-xl font-semibold">{item.title}</p>
						<p className="text-sm">{item.description}</p>
					</div>
					<Button onClick={() => router.push(`/cruise/${item.slug}`)}>See Details Cruise</Button>
				</div>
			)}
