		...
		<div>
			<div className="space-y-4 lg:space-y-0 lg:flex lg:items-end lg:gap-6">
				<div className="space-y-5 lg:space-y-0 w-full">
					<p className={cx("font-prata text-subtitle leading-snug")}>{packageData.title}</p>
				</div>
			...
			<div className={cx("pt-20 grid gap-4 transition-all duration-500 grid-cols-1 lg:grid-cols-12")}>
				{packageData.cruises.map((cruise, index) => {
	...
	return (
		<section id="explore">
			...
			<Container className={cx("flex flex-col gap-120-d")}>
				{CRUISE_PACKAGES.map((packageData, index) => (
					<CruisePackageSection key={packageData.slug} packageData={packageData} />
				))}
			</Container>
		</section>
	);
